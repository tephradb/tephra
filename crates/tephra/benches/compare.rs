//! Write-path comparison: tephra vs umadb, embedded (no gRPC).
//!
...
//! TEPHRA_BENCH_DIR=/mnt/nvme cargo bench --features umadb-compare --bench compare
//! ```
//!
...
//! (`tephra` vs `umadb`) so the bars sit side by side.
//!
//! ## What each engine is doing
//!
//! - **tephra**: `WriteHandle::append` hands the batch to the single writer thread, which
...
//! `TEPHRA_BENCH_DIR`, not a tmpfs, or the fsync ceiling is hidden.
//!
...
//! traversal, even when those bytes are warm in the OS cache, while tephra keeps its index
...
//! is no `group_commit` group here; tephra's concurrency story lives in `write_path`.
//!
//! ## The read comparison
//!
...
//!
//! - `read_full_scan`: read the whole log (`Query::all` / a `None` query). tephra's zero-copy
//!   sequential log scan vs umadb's in-order B+tree traversal, the highest-volume DCB read.
//! - `read_selectivity`: a tag query at graded selectivity (`1/1000` .. `1/10`). tephra's
...
//!   tail), the subscription-resume shape. Exercises tephra's whole-segment header pruning.
//! - `read_single_entity_and_payload`: one unique-tag read (load one aggregate), plus a
//!   payload-size sweep on a fixed query.
//!
//! Unlike the write-compare's 1 GiB segments, the read corpus uses small tephra segments so
...
//! would only flatter tephra). tephra runs with its default planner (`scan_bias`), so it is
...
//! `Vec<DcbSequencedEvent>` (owned, decoded events with payloads) before returning. tephra's
...
//! asymmetry (umadb allocates the whole result up front; tephra lends per event) is a real
...
mod imp {
    ...
    /// 1 GiB segments for tephra, matching its own bench so neither run rolls over mid-sample.
    ...
    /// even when those bytes are warm in the OS cache. tephra keeps its index fully resident
    ...
    /// A scratch directory, overridable via `TEPHRA_BENCH_DIR` so both engines land on the
    /// same physical device under test.
    fn scratch_dir() -> TempDir {
        match std::env::var_os("TEPHRA_BENCH_DIR") {
    ...
    /// A live tephra coordinator over a scratch directory, torn down on drop (writer thread
    ...
    fn tephra_event(seq: u64, payload: &[u8]) -> Event {
    ...
    fn append_latency(c: &mut Criterion) {
        ...
        {
            let engine = Tephra::new();
            group.bench_function("tephra", |b| {
                b.iter(|| {
                    let ev = tephra_event(next_seq(), &payload);
    ...
    fn batch_size(c: &mut Criterion) {
        ...
        for &batch in &[1usize, 8, 64, 512] {
            ...
            let engine = Tephra::new();
            let template = tephra_event(next_seq(), &payload);
            group.bench_with_input(BenchmarkId::new("tephra", batch), &batch, |b, &batch| {
                b.iter(|| {
    ...
    fn payload_size(c: &mut Criterion) {
        ...
        for &size in &[64usize, 256, 1024, 4096, 16384] {
            ...
            let engine = Tephra::new();
            group.bench_with_input(BenchmarkId::new("tephra", size), &size, |b, _| {
                b.iter(|| {
                    let ev = tephra_event(next_seq(), &payload);
                    black_box(engine.handle.append(vec![ev], None).expect("append"));
                });
    ...
    fn conditional_append(c: &mut Criterion) {
        ...
        {
            let engine = Tephra::new();
            group.bench_function("tephra_unconditional", |b| {
                b.iter(|| {
                    let ev = tephra_event(next_seq(), &payload);
                    black_box(engine.handle.append(vec![ev], None).expect("append"));
                });
        ...
        {
            let engine = Tephra::new();
            group.bench_function("tephra_unique_guard", |b| {
                b.iter(|| {
                    let n = next_seq();
    ...
    /// Small tephra segments so the corpus seals many on-disk segments: a store with history,
    ...
    fn tephra_read_event(seq: u64, payload: &[u8]) -> Event {
    ...
    impl Tephra {
        /// Builds a tephra store of [`READ_N`] events with `payload`-byte payloads, over small
        /// segments so the corpus seals many on-disk segments.
        fn corpus(payload_size: usize) -> Tephra {
            let dir = scratch_dir();
            ...
            for seq in 0..READ_N {
                batch.push(tephra_read_event(seq, &payload));
    ...
    impl Uma {
        ...
        /// workload tephra's [`Tephra::corpus`] builds.
    ...
    /// The umadb start bound for a tephra `after`: position 0 (the "before everything"
    ...
    /// Drains a tephra read to exhaustion, touching each event's payload, and returns the
    ...
    fn drain_tephra(handle: &WriteHandle, query: Query, after: Position) -> u64 {
    ...
    /// The ascending positions a tephra read yields.
    fn positions_tephra(handle: &WriteHandle, query: Query, after: Position) -> Vec<u64> {
        let mut reads = handle.read(query, after);
        let mut out = Vec::new();
        while let Some(item) = reads.next() {
    ...
    fn assert_same_result(
        ...
        let d = positions_tephra(handle, dcb_q, Position::new(after));
        let u = positions_uma(db, uma_q, uma_start(after));
        assert_eq!(
            d.len(),
            u.len(),
            "tephra ({}) and umadb ({}) disagreed on result count (after {after})",
            d.len(),
            u.len(),
        );
        assert_eq!(
            d, u,
            "tephra and umadb disagreed on the result positions (after {after})"
        );
    }

    /// Read everything: tephra's zero-copy sequential log scan vs umadb's in-order B+tree
    ...
    fn read_full_scan(c: &mut Criterion) {
        ...
        let tephra = Tephra::corpus(READ_PAYLOAD);
        let uma = Uma::corpus(READ_PAYLOAD);
        assert_same_result(&tephra.handle, &uma.db, Query::all(), None, 0);

        group.bench_function("tephra", |b| {
            b.iter(|| black_box(drain_tephra(&tephra.handle, Query::all(), Position::ZERO)));
        });
        group.bench_function("umadb", |b| {
    ...
    /// A tag query at graded selectivity: tephra's index plus per-match fetch vs umadb's
    ...
    fn read_selectivity(c: &mut Criterion) {
        ...
        for &d in &SELECTIVITY_DENOMS {
            ...
            assert_same_result(&tephra.handle, &uma.db, dq.clone(), uq.clone(), 0);

            let label = format!("1_over_{d}");
            group.bench_with_input(BenchmarkId::new("tephra", &label), &dq, |b, dq| {
                b.iter(|| black_box(drain_tephra(&tephra.handle, dq.clone(), Position::ZERO)));
            });
    ...
    /// Exercises tephra's whole-segment header pruning vs umadb's tree seek.
    fn read_range(c: &mut Criterion) {
        ...
        for &(label, after) in &[
            ...
            assert_same_result(&tephra.handle, &uma.db, dq.clone(), uq.clone(), after);

            group.bench_with_input(BenchmarkId::new("tephra", label), &after, |b, &after| {
                b.iter(|| black_box(drain_tephra(&tephra.handle, dq.clone(), Position::new(after))));
            });
    ...
    fn read_single_entity_and_payload(c: &mut Criterion) {
...

        {
            ...
            for &payload in &[64usize, 1024, 4096] {
                ...
                let tephra = Tephra::corpus(payload);
                let uma = Uma::corpus(payload);
                let dq = dcb_tag_query(&tag);
                let uq = uma_tag_query(&tag);
                assert_same_result(&tephra.handle, &uma.db, dq.clone(), uq.clone(), 0);

                group.bench_with_input(BenchmarkId::new("tephra", payload), &dq, |b, dq| {
                    b.iter(|| black_box(drain_tephra(&tephra.handle, dq.clone(), Position::ZERO)));
                });
