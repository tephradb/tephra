...
//! `dcbdb-core` and is re-exported here. This module owns the single definition of "does
...
pub use dcbdb_core::{AppendCondition, Query, QueryItem};

/// The match predicate over an encoded event.
///
/// This is an extension trait because [`Query`] and [`QueryItem`] are foreign types (they
/// live in `dcbdb-core`, so a client can build queries without the engine), while the
